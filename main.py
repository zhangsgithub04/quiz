...
def require_bearer_auth(authorization: Optional[str] = Header(default=None)):
    """
    Very simple Bearer token auth:
...
# In-memory "DB"
# -----------------------------
# A quiz bank: minimal example. Replace with your own.
...
# Session store:
# sessions[session_id] = {
#   quiz_id, mode, status, current_index, score,
#   served_order: [question_id...],
#   answers: [{question_id, selected, correct, answered_at}]
...
def create_session(req: CreateSessionRequest, _: bool = Depends(require_bearer_auth)):
    ...
    sessions[session_id] = {
        ...
        "mode": req.mode,
        "status": "active",
        "current_index": 0,  # next question index
        "score": 0,
        "served_order": [q["question_id"] for q in questions],  # fixed order for minimal example
        "answers": [],
        "started_at": started,
        "updated_at": started,
    }
...
def next_question(session_id: str, _: bool = Depends(require_bearer_auth)):
    ...
    # progress.current is 1-based question number being served
...
def submit_answer(session_id: str, req: SubmitAnswerRequest, _: bool = Depends(require_bearer_auth)):
    ...
    # Enforce that the answer corresponds to the *current* question
    ...
    # Reject duplicates (minimal rule). You could allow updates if desired.
    ...
    # Normalize selection (sort + unique)
    ...
    # progress.current = how many answered so far
